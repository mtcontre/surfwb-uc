...
for i in rango:
  ...
  plt.fill_between(x[1,:],z[1,:]+h[1,:],z[1,:],color='b')
  plt.fill_between(x[1,:],0*x[1,:],z[1,:],color='k')
  plt.fill_between(np.array([x.max(),x.max()+0.1]),np.array([0.,0.]),[0.12,0.12],color=3*[.5])
  plt.fill_between(np.array([x.min(),x.min()-0.1]),np.array([0.,0.]),[0.12,0.12],color=3*[.5])
  #plt.axis('equal')
  plt.xlim(-0.15,6.15)
  plt.ylim(0.,0.13)
