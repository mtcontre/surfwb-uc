...
%  x=reshape(s(:,1),nxi,neta);
%  y=reshape(s(:,2),nxi,neta);
%  z=reshape(s(:,3),nxi,neta);
%  h=zeros(size(x));
%  u=zeros(size(x));
%  v=zeros(size(x));
...
%  h=0.56-z;
%  h(h<0)=0;
b=load('data/SOL2D.0.dat');

x=reshape(b(:,1),nxi,neta);
y=reshape(b(:,2),nxi,neta);
z=reshape(b(:,3),nxi,neta);
h=reshape(b(:,4),nxi,neta);
u=reshape(b(:,5),nxi,neta);
v=reshape(b(:,6),nxi,neta);
