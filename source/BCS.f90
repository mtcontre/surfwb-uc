subroutine bcs(fopt,Cf,Coef,pasoRK,t,dt,Fr2,caso,qn,z,&
  ...
  use mpi_surf,only:myleft,myright,myback,myfront
  use mpi
  implicit none
...
!Eta=Ny, CB(4)
  ...
  SELECT CASE (CB(1))
    ...
    CASE(5)
      if (IO1==2) then 
      !Outflow, !Cota fija aguas afuera 
...
!---------------------------------------------------------------------------
  ...
  SELECT CASE (CB(2))
	...
	call INFLOWN_xi(pasoRK,fopt,Cf,Coef,Nx,Ny,Fr2,dxi,qsx2,qsy2,etas2,timeS2,&
	  Nsenal2,t,dt,qn,zt,xi,eta,qA2,zA2)
      else
...
! 	END DO
	...
	call INFLOW0_eta(pasoRK,fopt,Cf,Coef,Nx,Ny,Fr2,deta,qsx3,qsy3,etas3,&
	  timeS3,Nsenal3,t,dt,qn,zt,eta,xi,qA3,zA3)
      end if
