subroutine exchange_2d(qt,xit,etat,zt)
  use mpi
  use mpi_surf
  use global_variables,only:Nbx,Nby,outdir
  implicit none
...
subroutine  exchng_var_qxieta(var,dim1)
  ...
  call mpi_sendrecv(var(:,Nbx+1,3:Nby+2), 	dim1*Nby, 	mpi_double_precision,myfront,100, &	
		    var(:,1,3:Nby+2),		dim1*Nby,	mpi_double_precision,myback,100, &
...
subroutine  buffered_exchng_var_qxieta(var,dim1)
  use mpi
  use mpi_surf
  use global_variables,only:Nbx,Nby
  implicit none
