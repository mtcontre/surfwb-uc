...
SUBROUTINE fzero_B2(borde,Fr2,N,dep,epx,h,u,zep,epxA,hA,uA,dt,epR,epxR,uR,hR,zepR) ! Brent method
...
fo=epR0-N+dt*0.5D0*((uR0+sqrt(hR0/Fr2))*epxR0+(uA+sqrt(hA/Fr2))*epxA)
f=epR1-N+dt*0.5D0*((uR+sqrt(hR/Fr2))*epxR+(uA+sqrt(hA/Fr2))*epxA)
end if
...
!print*,'f(a) and f(b) of the same sign2'
END IF
c=b
fc=fb
...
!f=b-dt*0.5D0*((uR-sqrt(hR/Fr2))*epxR+(uA-sqrt(hA/Fr2))*epxA)

if (borde==1.OR.borde==3) then 

f=b-dt*0.5D0*((uR-sqrt(hR/Fr2))*epxR+(uA-sqrt(hA/Fr2))*epxA)
fo=b+dt*0.5D0*((uR0-sqrt(hR0/Fr2))*epxR0+(uA-sqrt(hA/Fr2))*epxA)

else
fo=b-N+dt*0.5D0*((uR0+sqrt(hR0/Fr2))*epxR0+(uA+sqrt(hA/Fr2))*epxA)
f=b-N+dt*0.5D0*((uR+sqrt(hR/Fr2))*epxR+(uA+sqrt(hA/Fr2))*epxA)
end if
