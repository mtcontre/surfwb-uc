...
!Xi=1, CB(1)
	...
	!Metricas
	DO j=3,Ny+2
	xit(:,1,j)=xi(:,1,j-2)		!xi-1=xi2
	xit(:,2,j)=xi(:,1,j-2)		!xi0=xi12
	
	etat(:,1,j)=eta(:,1,j-2)	!eta-1=eta2
...
SELECT CASE (CB(1))
	...
	qt(1,1,j)=qn(1,1,j-2)		!h-1=h2
	qt(1,2,j)=qn(1,1,j-2)		!h0=h1
	
	qt(2,1,j)=qn(2,1,j-2)		!u-1=-u2
	qt(2,2,j)=qn(2,1,j-2)		!u0=-u1
	
	qt(3,1,j)=qn(3,1,j-2)		!v-1=-v2
	qt(3,2,j)=qn(3,1,j-2)		!v0=-v1
...
!Xi=Nx, CB(2)
	...
	xit(:,Nx+4,j)=xi(:,Nx,j-2)	!xiNx+4=xiNx-1
	xit(:,Nx+3,j)=xi(:,Nx,j-2)	!xiNx+3=xiNx
	
	etat(:,Nx+4,j)=eta(:,Nx,j-2)	!etaNx+4=etaNx-1
...
SELECT CASE (CB(2))
	...
	qt(1,Nx+4,j)=qn(1,Nx,j-2)	!hNx+4=hNx-1
	qt(1,Nx+3,j)=qn(1,Nx,j-2)	!hNx+3=hNx
	
	qt(2,Nx+4,j)=qn(2,Nx,j-2)	!uNx+4=uNx-1
	qt(2,Nx+3,j)=qn(2,Nx,j-2)	!uNx+3=uNx
	
	qt(3,Nx+4,j)=qn(3,Nx,j-2)	!vNx+4=vNx-1
	qt(3,Nx+3,j)=qn(3,Nx,j-2)	!vNx+3=vNx
...
!Eta=1, CB(3)
	...
	!Metricas
	xit(:,i,1)=xi(:,i-2,1)		!xi-1=xi2
	xit(:,i,2)=xi(:,i-2,1)		!xi0=xi12
	
	etat(:,i,1)=eta(:,i-2,1)	!eta-1=eta2
...
SELECT CASE (CB(3)) 
	...
	qt(1,i,1)=qn(1,i-2,1)		!h-1=h2
	qt(1,i,2)=qn(1,i-2,1)		!h0=h1
	
	qt(2,i,1)=qn(2,i-2,1)		!u-1=u2
	qt(2,i,2)=qn(2,i-2,1)		!u0=u1
	
	qt(3,i,1)=qn(3,i-2,1)		!v-1=v2
	qt(3,i,2)=qn(3,i-2,1)		!v0=v1
...
!Eta=Ny, CB(4)
	...
	!Metricas
	xit(:,i,Ny+4)=xi(:,i-2,Ny)	!xiNy+4=xiNy-1
	xit(:,i,Ny+3)=xi(:,i-2,Ny)	!xiNy+3=xiNy
	
	etat(:,i,Ny+4)=eta(:,i-2,Ny)	!etaNy+4=etaNy-1
...
SELECT CASE (CB(4))
	...
	qt(1,i,Ny+4)=qn(1,i-2,Ny)	!hNy+4=hNy-1
	qt(1,i,Ny+3)=qn(1,i-2,Ny)	!hNy+3=hNy
	
	qt(2,i,Ny+4)=qn(2,i-2,Ny)	!uNy+4=-uNy-1
	qt(2,i,Ny+3)=qn(2,i-2,Ny)	!uNy+3=-uNy
	
	qt(3,i,Ny+4)=qn(3,i-2,Ny)	!vNy+4=-vNy-1
	qt(3,i,Ny+3)=qn(3,i-2,Ny)	!vNy+3=-vNy
